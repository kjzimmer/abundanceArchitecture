...
**Resolved 2026-09-25: per-visitor rate limiting.** Rate limiters key on Railway's `X-Real-IP`
(`server/src/lib/clientIp.ts`), not `trust proxy`. Measured in prod: X-Forwarded-For holds Cloudflare's
